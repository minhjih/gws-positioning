...
class ObjectPositionalEncoder(nn.Module):
    """
    object들의 x, y, z 좌표와 rotation(roll, pitch, yaw 등)을 포함한 [n,8] 입력을
    positional encoding을 통해 [n, dim] 임베딩으로 변환하는 클래스입니다.
    dim의 기본값은 64입니다.
    """
    def __init__(self, dim=64):
        super().__init__()
        self.dim = dim
        # 8차원 입력(x, y, z, rot1, x, y, z, rot2)에 대해 positional encoding 적용
        # 각 차원별로 별도의 주파수로 인코딩
        self.input_dim = 8
        self.pe_dim = dim // self.input_dim  # 각 입력 차원별로 할당할 임베딩 차원 수

        # 만약 dim이 8로 나누어떨어지지 않으면 마지막 차원에 나머지를 더함
        self.pe_dims = [self.pe_dim] * (self.input_dim - 1) + [dim - self.pe_dim * (self.input_dim - 1)]

    def positional_encoding(self, x, pe_dim):
        """
        x: [n] (1차원 벡터)
        pe_dim: 임베딩 차원 수
        return: [n, pe_dim]
        ...
        device = x.device
        n = x.shape[0]
        pe = torch.zeros(n, pe_dim, device=device)
        position = x.unsqueeze(1)  # [n, 1]
        div_term = torch.exp(torch.arange(0, pe_dim, 2, device=device) * (-np.log(10000.0) / pe_dim))
    ...
    def forward(self, obj_info):
        """
        obj_info: [n, 8] (x, y, z, rot1, x, y, z, rot2)
        return: [n, dim]
        """
        assert obj_info.shape[1] == self.input_dim, "입력은 [n,8]이어야 합니다."
        pe_list = []
        for i in range(self.input_dim):
            pe = self.positional_encoding(obj_info[:, i], self.pe_dims[i])
            pe_list.append(pe)
        out = torch.cat(pe_list, dim=1)  # [n, dim]
        #out = self.fc(out)
        return out
...
class DEGN_LIC:
    def __init__(self, n_pos=24, device='cuda'):
        ...
        '''
        self.E_OI = ObjectPositionalEncoder().to(device)
        self.E_OC = ObjectPositionalEncoder().to(device)
    ...
    # ------------------------ Step-1 ---------------------------------------
    def step1_train_location_encoder(self,x0,x1,labels,epochs=500,lr=1e-3,batch=16, pass_=True):
        if pass_ == False:
            print("\n[Step-1] Train E_L ...")
            ...
            for ep in range(epochs):
                ...
                if vacc>=best: best=vacc; torch.save(self.E_L.state_dict(),"our_models/best_E_L.pth")
                sched.step()
            self.E_L.load_state_dict(torch.load("our_models/best_E_L.pth"))
    ...
    # ------------------------ Step-2  (버그 fix 포함) ------------------------
    def step2_train_main_network(self,x0,x1,labels, p0, p1, epochs=300,batch=8,lr_g=1e-5,lr_d=2e-5, pass_=False):
        if(pass_ == False):
            print("\n[Step-2] Train full DEGN-LIC ...")
            loader = DataLoader(TensorDataset(x0,x1,p0,p1,labels),batch,True)
            opt_G = optim.Adam(
            ...
            for ep in range(epochs):
                ...
                for b,(xI,xC,pI,pC,_) in enumerate(loader):
                    xI,xC,pI,pC = xI.to(self.device), xC.to(self.device), pI.to(self.device), pC.to(self.device)
                    bs = xI.size(0)
                    ...
                    # 가우시안 노이즈 추가 (분산 0.1)
                    noise_sI = torch.randn_like(sI) * 0.1
                    noise_sC = torch.randn_like(sC) * 0.1
                    sI_r = sI + noise_sI
                    sC_r = sC + noise_sC
                    
                    xI_hat_det = self.G_I(lI,sI_r).detach()
                    xC_hat_det = self.G_C(lC,sC_r).detach()
                    '''
                    sC_r, sI_r = torch.randn_like(sC), torch.randn_like(sI)
                    여기에 랜덤한 벡터노이즈를 sI와 sC에 더해주는 방법도 생각해보자.
                    
                    '''
                    
                    xI2C_det = self.G_C(lI,sC).detach()
                    xC2I_det = self.G_I(lC,sI).detach()
                ...
                if G_sum/len(loader) < self.best_loss:
                        ...
                        torch.save(self.G_I.state_dict(),"our_models/best_G_I.pth")
                        torch.save(self.G_C.state_dict(),"our_models/best_G_C.pth")
                        '''
                        torch.save(self.E_SI.state_dict(),"our_models/best_E_SI.pth")
                        torch.save(self.E_SC.state_dict(),"our_models/best_E_SC.pth")
                        '''
                        torch.save(self.D_I.state_dict(),"our_models/best_D_I.pth")
                        torch.save(self.D_C.state_dict(),"our_models/best_D_C.pth")
                        print("Updated best model.", G_sum/len(loader))
            print("Step-2 finished.")


    # ------------------------ Step-3 ---------------------------------------
    def augment_data(self,x0,x1_pair,p0, p1,labels,ratio=2.0)->Tuple[torch.Tensor,torch.Tensor]:
        self.G_C.load_state_dict(torch.load("our_models/best_G_C.pth"))
        self.E_L.load_state_dict(torch.load("our_models/best_E_L.pth"))
        self.G_I.load_state_dict(torch.load("our_models/best_G_I.pth"))
        self.D_I.load_state_dict(torch.load("our_models/best_D_I.pth"))
        self.D_C.load_state_dict(torch.load("our_models/best_D_C.pth"))
        self.E_L.eval(); self.G_C.eval(); self.E_OI.eval(); self.E_OC.eval(); self.G_I.eval(); self.D_I.eval(); self.D_C.eval()
        print(f"\n[Step-3] Augment ×{ratio} ...")
        self.E_L.eval(); self.G_C.eval()
        aug_x,aug_y = [],[]
        with torch.no_grad():
            for xi,x1,p0i,p1i,lab in zip(x0,x1_pair,p0, p1, labels):
                xi = xi.unsqueeze(0).to(self.device)
                x1 = x1.unsqueeze(0).to(self.device)
                p0i = p0i.unsqueeze(0).to(self.device)
                p1i = p1i.unsqueeze(0).to(self.device)
                _,loc = self.E_L(x1)
                '''
    ...
    def save_model(self,path="our_models/degn_lic_scene0_to_scene1.pth"):
...
def load_real_data()->Tuple[torch.Tensor,torch.Tensor,torch.Tensor]:
    ...
    p0 = np.load('data/scene0_objects.npy'); p1 = np.load('data/scene1_objects.npy')
    ...
    return x0,x1_pair,labels, p0, p1
...
def visualize(org,org_1,aug,n=5):
    ...
    plt.tight_layout(); plt.savefig('our_results/augmentation_results.png'); plt.show()
...
def main():
    ...
    print("Device :",device)
    x0,x1_pair,labels, p0, p1 = load_real_data()
    ...
    model.step1_train_location_encoder(x0.to(device),x1_pair.to(device),
                                       labels.to(device))
    model.step2_train_main_network(x0.to(device),x1_pair.to(device),labels.to(device), p0.to(device), p1.to(device))
    aug_x,aug_y = model.augment_data(x0,x1_pair,p0, p1,labels,ratio=1.0)
    np.save('our_results/augmented_scene1_style.npy',aug_x.numpy())
    np.save('our_results/augmented_labels.npy',aug_y.numpy())
    model.save_model()
    visualize(x1_pair[60:511:90],x0[60:511:90],aug_x[60:511:90])
    print("\nFinished. Files generated:\n  • our_models/best_E_L.pth\n  • our_models/degn_lic_scene0_to_scene1.pth"
          "\n  • our_results/augmented_scene1_style.npy\n  • our_results/augmented_labels.npy\n  • our_results/augmentation_results.png")
